...
QUnit.test("startOffsetYears Test", function( assert ){
	var db = $('<input>').datebox({mode: "datebox", startOffsetYears: 2, defaultValue: new Date(2001,0,1)});

	db.trigger( "datebox", { method: "doset" } );

	assert.equal( db.val(), "2003-01-01", "Output should be ISO 2003-01-01 ("+db.val()+")" );
	
	db.datebox('destroy');
});

QUnit.test("startOffsetMonths Test", function( assert ){
	var db = $('<input>').datebox({mode: "datebox", startOffsetMonths: 2, defaultValue: new Date(2001,0,1)});

	db.trigger( "datebox", { method: "doset" } );

	assert.equal( db.val(), "2001-03-01", "Output should be ISO 2001-03-01 ("+db.val()+")" );
	
	db.datebox('destroy');
});

QUnit.test("startOffsetDays Test", function( assert ){
	var db = $('<input>').datebox({mode: "datebox", startOffsetDays: 2, defaultValue: new Date(2001,0,1)});

	db.trigger( "datebox", { method: "doset" } );

	assert.equal( db.val(), "2001-01-03", "Output should be ISO 2001-01-03 ("+db.val()+")" );
