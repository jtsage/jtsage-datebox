...
(function($) {
	$.widget( "mobile.datebox", $.mobile.widget, {
		...
		_event: function(e, p) {
			var jQmVer = parseInt($.mobile.version.replace(/\./g,''),10),
				w = jQmVer > 110
					? $(this).data('mobileDatebox')
					: $(this).data('datebox');
		...
		_create: function() {
			...
			var w = this,
				...
				jQmVer = parseInt($.mobile.version.replace(/\./g,''),10);
		...
		_applyCoords: function(e) {
			var w = e.widget,
				...
				pos = {
					x: (doc.w < 400 || o.centerHoriz || pp === 'window' )
						? (doc.w / 2) - (size.w /2)
						: iput.x  - (size.w / 2),
					y: (o.centerVert || pp === 'window' )
						? doc.t + ((doc.h / 2) - (size.h / 2))
						: iput.y  - ( size.h / 2 )
				};
			if ( typeof pp == 'string' && pp != 'window' )
			{
				pp = $(pp);
				if ( pp.length )
				{
					// determine center of popupPosition element
					pp = {
						x : pp.offset().left + pp.outerWidth() / 2,
						y : pp.offset().top + pp.outerHeight() / 2
					};
					// now center popup on popupPosition element
					pos = {
						x : pp.x - (size.w / 2),
						y : pp.y - (size.h / 2)
					}
	...
	$( document ).on( "pagecreate create", function( e ){
		...
		$( ":jqmData(role='datebox')", e.target ).each(function() {
			var jQmVer = parseInt($.mobile.version.replace(/\./g,''),10),
				defed = jQmVer > 111
					? typeof($(this).data('mobileDatebox'))
					: typeof($(this).data('datebox'));
			if ( defed === "undefined" ) {
				$(this).datebox();
			}
		});
